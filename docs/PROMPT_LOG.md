...
- **Date:** April 2026
- **Architecture:** Documented in `docs/PRICE_RESEARCH_AGENT.md`
- **Plan:**
  - Phase 1: Build with eBay Browse API (free, well-documented) — current target
  - Phase 2: Upgrade to Card Ladder API — contact [email protected] first to confirm API access under Pro subscription
  - Phase 3: Card Ladder web scrape fallback using session cookie — no additional cost
- **Key design:** `IPricingSource` interface so any source can be swapped in/out without rewriting agent logic
